...
# when run outside the project's virtual environment, delegate to the
# launcher script. this lets the user simply run this file and have the
# environment prepared automatically.
if __name__ == "__main__":
    script_dir = os.path.dirname(os.path.abspath(__file__))
    venv_dir = os.path.join(script_dir, ".venv")
    # crude check: see if current interpreter lives inside .venv
    if not os.path.commonpath([sys.executable, venv_dir]) == os.path.normpath(venv_dir):
        # hand off to launcher which will create the venv and re-run
...
def boot():
    # ensure log directory exists based on config (not cwd)
    logdir = os.path.dirname(config.LOG_PATH)
    ...
    parser = argparse.ArgumentParser(
        prog="runme",
        description="Lightweight OSINT utility for username/email lookups.",
    )
    parser.add_argument(
        "-u",
        "--username",
    ...
    # small helper to print rainbow checkmarks and symbols
    def _check(msg):
        config.console.print(f"[bold green]✓[/bold green] [bold cyan]{msg}[/bold cyan]")
    def _warn(msg):
        config.console.print(f"[bold yellow]⚠[/bold yellow]  [bold yellow]{msg}[/bold yellow]")
    def _error(msg):
        config.console.print(f"[bold red]✗[/bold red] [bold red]{msg}[/bold red]")
    config._check = _check
    config._warn = _warn
    config._error = _error
...
if __name__ == "__main__":
    boot()
    # compact banner: large project name and slightly bolder version with rainbow colors
    try:
        from rich.panel import Panel
        banner = Panel.fit(f"[bold magenta]▌[/bold magenta][bold rainbow]{config.PROJECT_NAME.replace('-', '_')}[/bold rainbow][bold magenta]▌[/bold magenta]\n[bold yellow]v{config.__version__}[/bold yellow]", style="bold cyan on black")
        config.console.print(banner)
    except Exception:
        config.console.print(f"[bold rainbow]{config.PROJECT_NAME.replace('-', '_')}[/bold rainbow] [bold yellow]v{config.__version__}[/bold yellow]")

    if config.about:
        config.console.print(
            """
        A small OSINT lookup tool
        """
        )
        sys.exit()
    ...
    if (
        ...
        config._error("Either --username or --email is required")
        sys.exit()
    if not config.username and (config.permute or config.permuteall):
        config._error("Permutations requires --username")
        sys.exit()

    if config.no_update:
        config.console.print("[bold cyan]→[/bold cyan] [bold yellow]Skipping update...[/bold yellow]")
    else:
        checkUpdates(config)
    ...
    # rest of processing (unchanged) follows
    
    if config.username_file:
        if isFile(config.username_file):
            config.username = getLinesFromFile(config.username_file)
            config.console.print(
                f"[bold green]✓[/bold green] [bold cyan]Successfully loaded {len(config.username)} usernames from \"{config.username_file}\"[/bold cyan]"
            )
        else:
            config._error(f'Could not read file "{config.username_file}"')
            sys.exit()
    ...
    if config.username:
        if (config.permute or config.permuteall) and len(config.username) > 1:
            ...
            config.console.print(
                f"[bold green]✓[/bold green] [bold cyan]Successfully loaded {len(config.username)} usernames from permuting {elements}[/bold cyan]"
            )
    ...
    if config.email_file:
        if isFile(config.email_file):
            ...
            config.console.print(
                f"[bold green]✓[/bold green] [bold cyan]Successfully loaded {len(config.email)} emails from \"{config.email_file}\"[/bold cyan]"
            )
        else:
            config._error(f'Could not read file "{config.email_file}"')
            sys.exit()
